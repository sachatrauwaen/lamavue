<template>
    <div>
        <vue-select v-if="showFolderSelector"
                    v-model="folder"
                    label="name"
                    ...
                    @input="folderChange"></vue-select>
        <vue-select v-if="showFileSelector"
                    v-model="model"
                    label="filename"
                    style="margin-bottom:10px"
                    :options="files"
            ...
            <template #option="{ filename, thumbUrl }">
                <div style="height:20px;width:20px;margin-right:5px;display:inline-block">
                    <img v-if="onlyImages" :src="thumbUrl" :alt="filename" class="img-fluid" />
                </div>
                <span>{{ filename }}</span>
            </template>
        </vue-select>
        <input v-if="showUpload"
               ref="input"
               type="file"
               name="image"
               :accept="accept"
               ...
               style="margin-bottom:10px" />
        <div v-if="showOverwrite" style=" position: absolute; bottom: 40px; right: 5px;">
