...
<script>
    ...
    let AddressField = {
        ...
        methods: {
            addressPart(place, adrtype) {
                var res = "";
                if (place && place.address_components) {
                    place.address_components.forEach((comp) => {
                        comp.types.forEach( (comptype) => {
                            if (comptype == adrtype) {
                                res = comp.long_name;
                                return;
                            }
    ...
    created() { },
    mounted() {
        if (!this.loaded && this.connector.googleApiKey) {
            this.loaded = true;
            const input = this.$el.querySelector('input');
            const loader = new Loader({
                apiKey: this.connector.googleApiKey,
                version: "weekly",
                libraries: ["places"]
            });
            loader.load().then(async () => {
                ...
                searchBox.addListener("places_changed", () => {
                    ...
                    let adr = {
                        postalcode: this.addressPart(place, "postal_code"),
                        city: this.addressPart(place, "locality"),
                        street: this.addressPart(place, "route"),
                        number: this.addressPart(place, "street_number"),
                        country: this.countryISO3(this.addressCountry(place, "country")),
